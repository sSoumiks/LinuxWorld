...
resource "aws_ebs_volume" "docu_ment" {
...
resource "aws_volume_attachment" "docu_ment_mount" {
    device_name = "/dev/xvdb"
    volume_id   = aws_ebs_volume.docu_ment.id
    ...
    provisioner "remote-exec" {
        inline  = [
            ...
            "sudo git clone https://github.com/sSoumiks/LinuxWorld.git /temp_repo",
...
resource "aws_s3_bucket" "image-bucket" {
    bucket  = "1my-2new-3bucket"
    acl     = "public-read"

    provisioner "local-exec" {
        command     = "git clone https://github.com/sSoumiks/LinuxWorld LinuxWorld"
    }

    provisioner "local-exec" {
        when        =   destroy
        command     =   "echo Y | rmdir /s LinuxWorld"
    }
}
